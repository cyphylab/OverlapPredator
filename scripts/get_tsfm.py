...
class PastaDataset(Dataset):
    ...
    def __init__(self, config, clouds_path):
        super(PastaDataset, self).__init__()
        self.config = config
        self.clouds_path = clouds_path
        self.offset = 0
        self.data = np.load(clouds_path)

    def __len__(self):
        return len(self.data) - self.offset
    
    def __getitem__(self, idx):
        src_pcd = self.data["arr_0"]
        tgt_pcd = self.data[f"arr_{self.offset+idx}"]
        src_pcd = np.hstack((src_pcd, np.zeros((src_pcd.shape[0], 1))))
        tgt_pcd = np.hstack((tgt_pcd, np.zeros((tgt_pcd.shape[0], 1))))
...
def main(config, demo_loader):
    ...
    with torch.no_grad():
        for i in range(len(demo_loader)):
            ...
            tsfms[i] = tsfm
            print(f"{i+1} of {len(demo_loader)} done")
        print(tsfms.shape)
        np.save("/mnt/d/lidar_experiments/pasta2d-main/data/tsfms", tsfms)
...
if __name__ == '__main__':
    ...
    demo_set = PastaDataset(config, config.csv_path)
